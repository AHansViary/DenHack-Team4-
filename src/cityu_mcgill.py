...
def jaccard_index(s1, s2) :
    ...
    intersect = intersectionjac(s1, s2);
...
import os
import pandas as pd
results = pd.DataFrame(columns=['cityu', 'mcgill', 'Similarity Index Jaccard', 'Similarity Index Cosine'])

import time

def math_model(intersect):
    if intersect > 5:
        return (-1/(intersect-(5/3)))+1
    else:
        return 7*intersect*intersect/250
...
def compare_files(dir1, dir2):
    ...
    results = pd.DataFrame(columns=['cityu', 'mcgill', 'Mathematical Model'])
    ...
    for i in range(len(files1)):
        for j in range(len(files2)):
            ...
            with open(file1, 'rb') as f1, open(file2, 'rb') as f2:
                ...
                results = results.append({'cityu': os.path.basename(file1), 'mcgill': os.path.basename(file2),  'Mathematical Model': math_model(inter)}, ignore_index=True)

    return results.to_excel("src/results_cityu_mcgill_math_model.xlsx", index=False)

#file1 = open("src\cityu\CityU_MA4524.pdf", "rb")
#pdf_file1 = PyPDF2.PdfReader(file1)

#file2 = open("McGill_MATH249.pdf", "rb")
#pdf_file2 = PyPDF2.PdfReader(file2)

#defining variables
#s1= normalizer(pdf_reader(pdf_file1))
#s2= normalizer(pdf_reader(pdf_file2))
#print("Math Model similarity = ",math_model(intersection(s1,s2)))
start_time = time.time()
compare_files('src\cityu', 'src\mcgill')
end_time = time.time()
